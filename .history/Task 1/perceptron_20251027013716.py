...
class PerceptronModel:
    ...
    def train(self, X_train, y_train):
        ...
        # Step 2 & 7: Repeat... Until stopping criterion (epochs) is reached
        for _ in range(self.epochs):
            
            # Step 3: For each training vector pair (x_i, t_i)
            # We loop using an index 'i'
            for i in range(n_samples):
                
                # Get the sample and its target label using the index
                x_i = X_train[i]
                target_t = y_train[i]
